...
const byName = (a, b) => (a.tradeName || a.name).localeCompare(b.tradeName || b.name, 'pt-BR')
...
function Field({ label, value, onChange, type = 'text', required = false, children, ...props }) {
  return <label className="field"><span>{label}{required && ' *'}</span>{children || <input type={type} value={value ?? ''} onChange={event => onChange(event.target.value)} required={required} {...props} />}</label>
}
...
function EntityForm({ item, fields, onSave }) { const [form, setForm] = useState(item); return <form onSubmit={event => { event.preventDefault(); onSave(form) }}><div className="form-grid">{fields.map(([key, label, type, options]) => <Field key={key} label={label} value={form[key]} onChange={value => setForm({ ...form, [key]: value })} type={type === 'number' ? 'number' : type || 'text'} step={type === 'number' ? '0.01' : undefined} min={type === 'number' ? '0' : undefined}>{type === 'select' && <select value={form[key]} onChange={event => setForm({ ...form, [key]: event.target.value })}>{options.map(x => <option key={x}>{x}</option>)}</select>}</Field>)}</div><div className="modal-actions"><button type="submit" className="primary"><Save size={17}/> Salvar</button></div></form> }
...
function QuoteForm({ item, db, user, onSave, onNewPayment }) {
  ...
  return <form onSubmit={event => { event.preventDefault(); onSave({ ...form, discount: numeric(form.discount), items: form.items.map(x => ({ ...x, qty: numeric(x.qty), price: numeric(x.price) })) }) }}>
    <div className="section-label">Dados do pedido</div><div className="form-grid">
      ...
      <Field label="Cliente" value={form.clientId} onChange={() => {}}><select required value={form.clientId} onChange={e => change('clientId', e.target.value)}><option value="">Selecione...</option>{db.clients.slice().sort(byName).map(c => <option key={c.id} value={c.id}>{c.tradeName || c.name} {c.document && `· ${c.document}`}</option>)}</select></Field>
    ...
    <div className="section-label">Pagamento e preços</div><p className="help">Comissão padrão configurada: <strong>{Number(db.settings.commissionRate ?? 0).toLocaleString('pt-BR')}%</strong>. Você poderá ajustar essa taxa ao registrar cada faturamento.</p><div className="form-grid">
      ...
      <Field label="Desconto em R$" type="number" step="0.01" min="0" value={form.discount} onChange={x => change('discount', x)}/>
    ...
    <div className="section-label">Produtos</div><div className="inline-add"><select value={productId} onChange={e => setProductId(e.target.value)}><option value="">Selecione um produto...</option>{db.products.filter(p => p.active).map(p => <option key={p.id} value={p.id}>{p.name} · {money(form.priceMode === 'cash' ? p.cash : p.term)}{p.review ? ' · revisar preço' : ''}</option>)}</select><button type="button" className="secondary" onClick={addProduct}><Plus size={16}/> Adicionar</button></div>
    <div className="table-wrap quote-items"><table><thead><tr><th>Produto</th><th>Qtd.</th><th>Unid.</th><th>Preço unit.</th><th>Total</th><th></th></tr></thead><tbody>{form.items.map(x => <tr key={x.id}><td><input value={x.name} onChange={e => updateItem(x.id, 'name', e.target.value)}/></td><td><input className="narrow" type="number" step="0.01" min="0.01" value={x.qty} onChange={e => updateItem(x.id, 'qty', e.target.value)}/></td><td><input className="narrow" value={x.unit} onChange={e => updateItem(x.id, 'unit', e.target.value)}/></td><td><input className="narrow" type="number" step="0.01" min="0" value={x.price} onChange={e => updateItem(x.id, 'price', e.target.value)}/></td><td>{money(numeric(x.qty) * numeric(x.price))}</td><td><button type="button" className="icon-btn" onClick={() => setForm(current => ({ ...current, items: current.items.filter(i => i.id !== x.id) }))}><Trash2 size={16}/></button></td></tr>)}</tbody></table>{!form.items.length && <Empty text="Adicione os produtos do orçamento."/>}</div>
